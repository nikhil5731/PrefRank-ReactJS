...
const Modal = ({ toggleModal, selected, setSelected, branches, states }) => {
  // const statesInIndia = [
  //   "Andhra Pradesh",
  //   "Arunachal Pradesh",
  //   "Assam",
  ...
  const handleClick = (searchInput, key) => {
    if (selected[key].includes(searchInput)) {
      const updatedArray = selected[key].filter(
        (element) => element !== searchInput
      );
      setSelected({ ...selected, [key]: updatedArray });
    } else {
      const updatedArray = [...selected[key], searchInput];
      setSelected({ ...selected, [key]: updatedArray });
    }
  };
                ...
                <div className="bg-[#C4DAFF] h-96 p-5 rounded-xl overflow-y-scroll flex flex-col gap-2">
                  {branches.map((ele, index) => (
                    ...
                    >
                      ...
                      <div
                        className={`${
                          selected["branches"].includes(ele)
                            ? "bg-[#334C8A] text-white"
                            : "bg-white"
                        } w-full py-3 rounded-xl text-center border border-black italic`}
                      >
